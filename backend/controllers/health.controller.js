...
const list = asyncHandler(async (req, res) => {
  const records = healthService.listForOwner(req.user.id);
  sendSuccess(res, 200, { records });
});

// GET /api/pets/:petId/health-records
const listForPet = asyncHandler(async (req, res) => {
  const records = healthService.listForPet(req.user.id, req.params.petId);
  sendSuccess(res, 200, { records });
});

// POST /api/pets/:petId/health-records
const createForPet = asyncHandler(async (req, res) => {
  const record = healthService.createForPet(req.user.id, req.params.petId, req.body || {});
  sendSuccess(res, 201, { record });
});

// PUT /api/health-records/:id
const update = asyncHandler(async (req, res) => {
  const record = healthService.updateForOwner(req.user.id, req.params.id, req.body || {});
  sendSuccess(res, 200, { record });
});

// DELETE /api/health-records/:id
const remove = asyncHandler(async (req, res) => {
  healthService.removeForOwner(req.user.id, req.params.id);
